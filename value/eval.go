...
func binaryVectorOp(c Context, i Value, op string, j Value) Value {
	...
	// Try SIMD implementation for equal-length vectors
	// SIMD functions return nil if not applicable (wrong types, SIMD not enabled, etc.)
	if u.Len() == v.Len() && u.Len() > 0 {
		var simdResult *Vector
		switch op {
		case "+":
			simdResult = vectorAddSIMD(c, u, v)
		case "-":
			simdResult = vectorSubSIMD(c, u, v)
		case "*":
			simdResult = vectorMulSIMD(c, u, v)
		case "min":
			simdResult = vectorMinSIMD(c, u, v)
		case "max":
			simdResult = vectorMaxSIMD(c, u, v)
		}
		if simdResult != nil {
			return simdResult
		}
